...
static enum clock_control_status stm32_clock_control_get_status(const struct device* dev,
    ...
    if (IN_RANGE(pclken->bus, STM32_PERIPH_BUS_MIN, STM32_PERIPH_BUS_MAX) == true) {
        /* Gated clocks */
        if ((sys_read32(STM32H7_BUS_CLK_REG + pclken->bus) & pclken->enr)
            == pclken->enr) {
...
static int set_up_plls(void) {
    ...
    const uint32_t vco1_ck = PLLX_VCO_FREQ(
        PLLSRC_FREQ, (uint64_t)STM32_PLL_M_DIVISOR,
    ...
    #if defined(STM32_PLL2_ENABLED)
    uint32_t const vco2_ck = PLLX_VCO_FREQ(
        PLLSRC_FREQ, (uint64_t)STM32_PLL2_M_DIVISOR,
    ...
    #if defined(STM32_PLL3_ENABLED)
    uint32_t const vco3_ck = PLLX_VCO_FREQ(
        PLLSRC_FREQ, (uint64_t)STM32_PLL3_M_DIVISOR,
