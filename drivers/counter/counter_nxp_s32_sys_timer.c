...
static int stm_set_alarm(const struct device* dev, uint8_t channel, uint32_t ticks, uint32_t flags) {
    ...
    uint32_t const now = REG_READ(STM_CNT);
    uint32_t const top_val = config->info.max_top_value;
    int err = 0;
    uint32_t diff;
...
static void nxp_s32_stm_isr(const struct device* dev) {
    const struct nxp_s32_sys_timer_config* config = dev->config;
    struct nxp_s32_sys_timer_data* data = dev->data;
    ...
    for (size_t channel = 0; channel < SYS_TIMER_NUM_CHANNELS; ++channel) {
...
static uint32_t nxp_s32_sys_timer_get_pending_int(const struct device* dev) {
    ...
    for (size_t i = 0; i < counter_get_num_of_channels(dev); i++) {
...
#define SYS_TIMER_INIT_DEVICE(n)                                        \
    static int nxp_s32_sys_timer_##n##_init(const struct device* dev) { \
        IRQ_CONNECT(DT_INST_IRQN(n), DT_INST_IRQ(n, priority),          \
                    nxp_s32_stm_isr, DEVICE_DT_INST_GET(n),             \
    ...
    static struct nxp_s32_sys_timer_data nxp_s32_sys_timer_data_##n;    \
                                                                        \
    static struct nxp_s32_sys_timer_config DT_CONST nxp_s32_sys_timer_config_##n = { \
        .info = {                                                       \
        ...
        .irqn         = DT_INST_IRQN(n),                                \
                                                                        ...
                                                                        \
    DEVICE_DT_INST_DEFINE(n, nxp_s32_sys_timer_##n##_init,              \
                          NULL,                                         \
