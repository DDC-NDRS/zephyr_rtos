 ...
 *
 * Small driver-internal scratch (key, IV, tag, CTR counter,
...
struct mxcrypto_session {
	 ...
	 * driver-built data that the caller does not own (CTR counter
	 * block).
	 */
...
static struct mxcrypto_session *mxcrypto_session_alloc(const struct device *dev)
{
	...
	if (ret < 0) {
		LOG_ERR("Failed to acquire MXCRYPTO lock for alloc: %d", ret);
		return NULL;
	}
				  ...
				  struct mxcrypto_session *session)
{
	...
	ret = mfd_infineon_mxcrypto_lock(cfg->mfd, K_FOREVER);
	if (ret < 0) {
		LOG_ERR("Failed to acquire MXCRYPTO lock for free: %d", ret);
		return;
	}
...
static int mxcrypto_aes_ecb_op(struct cipher_ctx *ctx, struct cipher_pkt *pkt)
{
	...
	ret = mfd_infineon_mxcrypto_lock(cfg->mfd, K_FOREVER);
	if (ret < 0) {
		LOG_ERR("Failed to acquire MXCRYPTO lock for ECB: %d", ret);
		return -EIO;
	}
			       ...
			       uint8_t *iv)
{
	...
	ret = mfd_infineon_mxcrypto_lock(cfg->mfd, K_FOREVER);
	if (ret < 0) {
		LOG_ERR("Failed to acquire MXCRYPTO lock for CBC: %d", ret);
		return -EIO;
	}
			       ...
			       uint8_t *iv)
{
	...
	/*
	 * The PDL CTR core only supports a 32-bit counter correctly.
	 */
	if (ctr_len_bytes != 4U) {
		LOG_ERR("AES CTR: only a 32-bit counter is supported");
		return -EINVAL;
	}
	...
	ret = mfd_infineon_mxcrypto_lock(cfg->mfd, K_FOREVER);
	if (ret < 0) {
		LOG_ERR("Failed to acquire MXCRYPTO lock for CTR: %d", ret);
		return -EIO;
	}

	/*
	 * Compose the 16-byte initial counter block (nonce || zero
	 * counter) in driver-owned nocache scratch.  This is
	 * driver-built; it is not a caller buffer.
	 */
	...
	/*
	 * Use the multipart CTR API: the single-shot Cy_Crypto_Core_Aes_Ctr
	 * only encrypts whole 16-byte blocks and drops any trailing partial
	 * block, whereas CTR must process the tail as a stream.
	 */
	rc = Cy_Crypto_Core_Aes_Ctr_Setup(base, &session->aes_state);
	if (rc == CY_CRYPTO_SUCCESS) {
		rc = Cy_Crypto_Core_Aes_Ctr_Set_IV(base, session->aes_iv_sram,
						   &session->aes_state);
	}
	if (rc == CY_CRYPTO_SUCCESS) {
		rc = Cy_Crypto_Core_Aes_Ctr_Update(base, pkt->in_len,
						   pkt->out_buf, pkt->in_buf,
						   &session->aes_state);
	}
	if (rc == CY_CRYPTO_SUCCESS) {
		rc = Cy_Crypto_Core_Aes_Ctr_Finish(base, &session->aes_state);
	}
			       ...
			       struct cipher_aead_pkt *aead, uint8_t *nonce)
{
	...
	ret = mfd_infineon_mxcrypto_lock(cfg->mfd, K_FOREVER);
	if (ret < 0) {
		LOG_ERR("Failed to acquire MXCRYPTO lock for CCM: %d", ret);
		return -EIO;
	}
			       ...
			       struct cipher_aead_pkt *aead, uint8_t *nonce)
{
	...
	ret = mfd_infineon_mxcrypto_lock(cfg->mfd, K_FOREVER);
	if (ret < 0) {
		LOG_ERR("Failed to acquire MXCRYPTO lock for GCM: %d", ret);
		return -EIO;
	}
						     ...
						     struct mxcrypto_session *session)
{
	...
	rc = Cy_Crypto_Core_Sha_Start(base, &session->sha_state);
	if (rc == CY_CRYPTO_SUCCESS) {
		session->sha_started = true;
	}
...
static int mxcrypto_hash_op(struct hash_ctx *ctx, struct hash_pkt *pkt, bool finish)
{
	...
	ret = mfd_infineon_mxcrypto_lock(cfg->mfd, K_FOREVER);
	if (ret < 0) {
		LOG_ERR("Failed to acquire MXCRYPTO lock for hash: %d", ret);
		return -EIO;
	}

	if (!session->sha_started) {
				       ...
				       enum hash_algo algo)
{
	...
	ret = mfd_infineon_mxcrypto_lock(cfg->mfd, K_FOREVER);
	if (ret < 0) {
		LOG_ERR("Failed to acquire MXCRYPTO lock for SHA init: %d", ret);
		return -EIO;
	}
	rc = mxcrypto_sha_init_start(base, session);
	mfd_infineon_mxcrypto_unlock(cfg->mfd);
...
static int mxcrypto_hash_free_session(const struct device *dev, struct hash_ctx *ctx)
{
	...
	ret = mfd_infineon_mxcrypto_lock(cfg->mfd, K_FOREVER);
	if (ret < 0) {
		LOG_ERR("Failed to acquire MXCRYPTO lock for SHA free: %d", ret);
		return -EIO;
	}
	if (session->sha_started) {
		(void)Cy_Crypto_Core_Sha_Free(base, &session->sha_state);
