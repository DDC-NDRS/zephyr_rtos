...
#define EEPROM_MAX_PG_SZ        64U         /* Support maximum page size of 64 bytes */
                                            /* Known maximum page size is 128 bytes for 512-Kbit EEPROM */
                                            /* Known maximum page size is 256 bytes for 1-Mbit, 2-Mbit EEPROM */

#define EEPROM_MAX_ADDR_WIDTH   4U          /* Support maximum address width of 3 bytes (+1 as 4 bytes align) */
...
static size_t eeprom_at2x_limit_write_count(const struct device* dev,
    ...
    if ((off_t)(offset + count) > page_boundary) {
...
static uint16_t eeprom_at24_translate_offset(const struct device* dev,
    ...
    const uint16_t addr_incr = (uint16_t)(*offset >> config->addr_width);
...
static int eeprom_at24_read(const struct device* dev, off_t offset, void* buf,
    ...
    if (config->addr_width == 16) {
        sys_put_be16((uint16_t)offset, addr);
    }
    else {
...
static int eeprom_at24_write(const struct device* dev, off_t offset,
    ...
    count    = eeprom_at2x_limit_write_count(dev, offset, len);
    bus_addr = eeprom_at24_translate_offset(dev, &offset);
...
/* #CUSTOM@NDRS */
#define ASSERT_SIZE_PAGESIZE_LIMIT(page)                        \
    BUILD_ASSERT((page <= EEPROM_MAX_PG_SZ), "Page size shall <= 64 bytes")
...
#define EEPROM_AT2X_DEVICE(n, t)                                \
    ...
    ASSERT_SIZE_PAGESIZE_LIMIT(DT_PROP(INST_DT_AT2X(n, t), pagesize));      \
