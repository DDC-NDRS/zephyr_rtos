...
mtb_hal_clock_t const CYBSP_SMIF_CORE_0_XSPI_FLASH_hal_clock = {
...
cy_stc_syspm_callback_t flash_deep_sleep = {
    ifx_serial_memory_flash_pm_callback,
    CY_SYSPM_DEEPSLEEP,
...
static int ifx_serial_memory_hw_init(void) {
    ...
    Cy_SMIF_Reset_Memory(IFX_SERIAL_MEMORY_SMIF,
                         smif0BlockConfig.memConfig[0]->slaveSelect);
    ...
    Cy_SMIF_SetDataSelect(IFX_SERIAL_MEMORY_SMIF,
                          smif0BlockConfig.memConfig[0]->slaveSelect,
...
static int ifx_serial_memory_flash_init(const struct device* dev) {
    ...
    cy_rslt_t result = mtb_serial_memory_setup(
        &serial_memory_obj, BOARD_SMIF_CHIP_SELECT, IFX_SERIAL_MEMORY_SMIF,
...
static struct ifx_serial_memory_flash_config DT_CONST flash_config = {
