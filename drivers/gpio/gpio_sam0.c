...
static int gpio_sam0_config(const struct device* dev,
                            gpio_pin_t pin,
                            gpio_flags_t flags) {
...
static int gpio_sam0_port_set_masked_raw(const struct device* dev,
                                         ...
                                         gpio_port_value_t value) {
    const struct gpio_sam0_config* config = dev->config;
    uint32_t out = config->regs->xOUT.reg;

    config->regs->xOUT.reg = (out & ~mask) | (value & mask);
        ...
        case GPIO_INT_MODE_EDGE :
            ...
            if (rc == 0) {
                rc = sam0_eic_acquire(config->id, pin, trigger,
                                      ((data->debounce & BIT(pin)) != 0), gpio_sam0_isr, data);
            }
...
void zephyr_gpio_sam0_set_regs(const struct gpio_dt_spec* spec, volatile void* reg_ptr) {
    struct gpio_sam0_config* config = (struct gpio_sam0_config*)spec->port->config;
