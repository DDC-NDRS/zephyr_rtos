...
bool i2c_stm32_xfer_will_use_dma(struct i2c_stm32_config const* cfg, void* buf, size_t len, bool tx) {
...
static void i2c_stm32_bitbang_set_scl(void* io_context, int state) {
    struct i2c_stm32_config const* config = io_context;
...
int i2c_stm32_recover_bus(struct device const* dev) {
    ...
    struct i2c_stm32_config const* config = dev->config;
...
void i2c_stm32_dma_tx_cb(struct device const* dma_dev __unused, void* user_data __unused,
...
void i2c_stm32_dma_rx_cb(struct device const* dma_dev __unused, void* user_data __unused,
                         uint32_t channel __unused, int status) {
    /* log DMA RX error */
    if (status != 0) {
        LOG_ERR("DMA error %d", status);
...
#define STM32_I2C_CFG_REG_INIT(index) zephyr_i2c_cfg_reg_init(&i2c_stm32_cfg_##index);
