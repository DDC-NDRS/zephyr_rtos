...
void modem_cellular_chat_callback_handler(struct modem_chat* chat,
    ...
    if (result == MODEM_CHAT_SCRIPT_RESULT_SUCCESS) {
        ...
        modem_cellular_delegate_event_ptr(data, MODEM_CELLULAR_EVENT_SCRIPT_FAILED,
                                          script);
    }
}
...
static void modem_cellular_delegate_event(struct modem_cellular_data* data,
                                          enum modem_cellular_event evt) {
    modem_cellular_delegate_event_ptr(data, evt, NULL);
}
    ...
    switch (evt) {
        ...
        case MODEM_CELLULAR_EVENT_PERIODIC_KICK :
            ...
            if (modem_chat_run_script_async(&data->chat,
                                            config->vendor->scripts.periodic) < 0) {
