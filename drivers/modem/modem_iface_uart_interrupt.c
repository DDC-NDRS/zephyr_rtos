...
static void modem_iface_uart_isr(const struct device* uart_dev,
    ...
    while (true) {
        uint32_t partial_size = ring_buf_put_ptr(&data->rx_rb, &dst, 0);
        if (!partial_size) {
        ...
        int rx = uart_fifo_read(ctx->iface.dev, dst, partial_size);
        if (rx <= 0) {
            break;
        }
...
static int modem_iface_uart_read(struct modem_iface* iface,
    ...
    *bytes_read = ring_buf_get(&data->rx_rb, buf, size);
    ...
    if (data->hw_flow_control && (*bytes_read == 0U)) {
