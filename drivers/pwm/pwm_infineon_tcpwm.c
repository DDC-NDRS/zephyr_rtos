...
static int ifx_tcpwm_capture_init(const struct device *dev)
{
	 ...
	 * CY_TCPWM_INPUT_TRIG_0), not guessed.
	 */
	const cy_stc_tcpwm_counter_config_t capture_config = {
	...
	status = IFX_TCPWM_Counter_Init(config->reg_base, &capture_config);
	if (status != CY_TCPWM_SUCCESS) {
		return -ENOTSUP;
	}
...
static void ifx_tcpwm_pwm_capture_isr(const struct device *dev, uint32_t pending)
{
	...
	if ((pending & CY_TCPWM_INT_ON_CC1) != 0) {
		uint32_t falling = IFX_TCPWM_Counter_GetCapture1Val(config->reg_base);
	...
	if ((pending & CY_TCPWM_INT_ON_CC0) != 0) {
		uint32_t rising = IFX_TCPWM_Counter_GetCapture0Val(config->reg_base);
