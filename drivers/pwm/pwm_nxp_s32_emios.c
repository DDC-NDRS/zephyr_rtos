...
static int pwm_nxp_s32_set_cycles_opwfmb(const struct device* dev, uint32_t channel,
    ...
    if ((ch_data->inverted != flags) || (!ch_data->curr_period)) {
        ...
        emios->CH.UC[channel].C &= ~(eMIOS_C_MODE_MASK | eMIOS_C_EDPOL_MASK);
        emios->CH.UC[channel].A = pulse_cycles;
        emios->CH.UC[channel].B = period_cycles;
         ...
         */
        if (pulse_cycles) {
            emios->CH.UC[channel].C |= eMIOS_C_EDPOL(polarity);
        }
        else {
            emios->CH.UC[channel].C |= eMIOS_C_EDPOL(!polarity);
        }
        ...
        key = irq_lock();
        emios->CH.UC[channel].C |= eMIOS_C_MODE(EMIOS_PWM_IP_MODE_OPWFMB_FLAG);

        if (pulse_cycles) {
            /* Restore expected value for Cn[EDPOL] */
            emios->CH.UC[channel].C =
                (emios->CH.UC[channel].C & ~(eMIOS_C_EDPOL_MASK)) | eMIOS_C_EDPOL(!polarity);
        }
        irq_unlock(key);
...
static int pwm_nxp_s32_set_cycles_opwmcb(const struct device* dev, uint32_t channel,
    ...
    if ((ch_data->inverted != flags) || (!ch_data->curr_period)) {
        ...
        if (master_ch_data->curr_period != period_cycles) {
             ...
             * when needed.
             */
            emios->CH.UC[ch_data->master_channel].C &= ~eMIOS_C_MODE_MASK;
            emios->CH.UC[ch_data->master_channel].A = period_cycles;
        }

        emios->CH.UC[channel].C &= ~(eMIOS_C_MODE_MASK  |
                                     eMIOS_C_EDPOL_MASK |
                                     eMIOS_C_BSL_MASK);
        ...
        emios->CH.UC[channel].A = pulse_cycles;
        emios->CH.UC[channel].B = pwm_info->DeadTime;

        key = irq_lock();
        emios->CH.UC[channel].C |= (eMIOS_C_MODE(pwm_info->Mode) |
        ...
        if (master_ch_data->curr_period != period_cycles) {
            emios->CH.UC[ch_data->master_channel].C |=
...
static int pwm_nxp_s32_set_cycles_opwmb(const struct device* dev, uint32_t channel,
    ...
    if ((ch_data->inverted != flags) || (!ch_data->curr_period)) {
        ...
        emios->CH.UC[channel].A = pwm_info->PhaseShift;
        emios->CH.UC[channel].B = pulse_cycles;
        ...
        if (!pwm_info->PhaseShift) {
             ...
             */
            emios->CH.UC[channel].C |= eMIOS_C_FORCMA(1);
        }
        irq_unlock(key);
...
static int pwm_nxp_s32_set_cycles(const struct device* dev, uint32_t channel,
    ...
    switch (pwm_info->Mode) {
        #if defined(EMIOS_PWM_IP_MODE_OPWFMB_USED)
...
static int pwm_nxp_s32_capture_configure(const struct device* dev,
    ...
    if (emios->CH.UC[channel].C & eMIOS_C_FEN_MASK) {
        LOG_ERR("Channel %d is busy", channel);
        return -EBUSY;
    }

    struct pwm_nxp_s32_channel_data* ch_data = &data->ch_data[channel];
    ch_data->continuous     = (flags & PWM_CAPTURE_MODE_MASK);
    ch_data->inverted       = (flags & PWM_POLARITY_MASK);
    ch_data->pulse_capture  = (flags & PWM_CAPTURE_TYPE_PULSE);
    ch_data->period_capture = (flags & PWM_CAPTURE_TYPE_PERIOD);
    ch_data->callback       = cb;
    ch_data->user_data      = user_data;
...
static int pwm_nxp_s32_capture_enable(const struct device* dev, uint32_t channel) {
    ...
    if (ch_data->period_capture && !ch_data->pulse_capture) {
        num_edge = 2U;
...
static int pwm_nxp_s32_get_cycles_per_sec(const struct device* dev,
    ...
    eMIOS_Type* emios = config->emios;
    struct pwm_nxp_s32_data* data = dev->data;
    uint8_t internal_prescaler;
    uint8_t global_prescaler;
    uint8_t master_channel;

    master_channel = data->ch_data[channel].master_channel;
    internal_prescaler = (emios->CH.UC[master_channel].C2 & eMIOS_C2_UCEXTPRE_MASK) >>
                          eMIOS_C2_UCEXTPRE_SHIFT;

    /* Clock source for internal prescaler is from either eMIOS or eMIOS / global prescaler */
    if (emios->CH.UC[master_channel].C2 & eMIOS_C2_UCPRECLK_MASK) {
        *cycles = data->emios_clk / (internal_prescaler + 1);
    }
    else {
        global_prescaler = (emios->MCR & eMIOS_MCR_GPRE_MASK) >> eMIOS_MCR_GPRE_SHIFT;
...
static void pwm_nxp_s32_capture_callback(const struct device* dev, uint32_t channel) {
    ...
    if (ch_data->period_capture && !ch_data->pulse_capture) {
        period = pwm_nxp_s32_capture_calc(ch_data->edge_buff[0],
                                          ch_data->edge_buff[1]);
    }
    else {
        if (ch_data->pulse_capture) {
            pulse = pwm_nxp_s32_pulse_calc(ch_data->inverted,
                                           ch_data->edge_buff,
                                           Emios_Icu_Ip_GetInputLevel(config->instance,
                                                                      channel));
        }

        if (ch_data->period_capture) {
            period = pwm_nxp_s32_capture_calc(ch_data->edge_buff[0],
                                              ch_data->edge_buff[2]);
        }
    }

    if (!ch_data->continuous) {
        Emios_Icu_Ip_StopTimestamp(config->instance, channel);
    }

    ch_data->callback(dev, channel, period, pulse, 0,
                      ch_data->user_data);
}
#endif
...
static int pwm_nxp_s32_init(const struct device* dev) {
    ...
    #if EMIOS_PWM_IP_USED
    ret = pwm_nxp_s32_pulse_gen_init(dev);
    if (ret < 0) {
        return ret;
    }
    #endif
    ...
    #ifdef CONFIG_PWM_CAPTURE
    ret = pwm_nxp_s32_pulse_capture_init(dev);
    if (ret < 0) {
        return ret;
    }
    #endif
...
#define PWM_NXP_S32_INIT_DEVICE(n)                              \
    ...
    static struct pwm_nxp_s32_config DT_CONST pwm_nxp_s32_config_##n = {        \
        .emios        = (eMIOS_Type*)DT_REG_ADDR(DT_INST_PARENT(n)),            \
