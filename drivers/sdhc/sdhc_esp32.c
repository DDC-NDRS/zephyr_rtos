				     ...
				     struct sdmmc_command *cmdinfo)
{
	...
	if (sdhc_esp32_ctrl.owner != dev) {
		if (sdhc_esp32_ctrl.owner != NULL && data->host_div != 0) {
			...
			sdmmc_ll_clear_interrupt(sdio_hw, 0xFFFF);
...
/*
 * Slots wired to dedicated IO_MUX pins are configured through the fixed IO_MUX
 * function. Slots with no IO_MUX pins (ESP32-P4 slot 1, every ESP32-S3 slot)
 * must be routed through the GPIO matrix using the per-slot signal indices.
 */
static void configure_slot_pins(const struct sdhc_esp32_config *cfg)
{
#if defined(SDHC_ESP32_USE_GPIO_MATRIX)
 ...
 */
static int sdhc_esp32_reset_ctrl(const struct device *dev)
{
	const struct sdhc_esp32_config *cfg = dev->config;
	sdmmc_dev_t *sdio_hw = (sdmmc_dev_t *)cfg->sdio_hw;
...
/*
 * Restore the controller registers cleared by a reset. The interrupt mask is
 * shared, so the other slot's card-interrupt bit is preserved. That bit is
 * armed under an interrupt lock, so the scan and the write take the same lock.
 */
static void sdhc_esp32_restore_after_reset(const struct device *dev)
{
	const struct sdhc_esp32_config *cfg = dev->config;
	sdmmc_dev_t *sdio_hw = (sdmmc_dev_t *)cfg->sdio_hw;
	uint32_t keep_io = 0;
	...
	for (int s = 0; s < SOC_SDMMC_NUM_SLOTS; s++) {
		...
		if (other_data->sdio_int_sources != 0) {
			keep_io |= (s == 0) ? SDMMC_LL_EVENT_IO_SLOT0 : SDMMC_LL_EVENT_IO_SLOT1;
		}
	}
#endif
...
/*
 * The reset is controller wide, so it must not run while the other slot owns
 * the bus, and it clears the shared divider, interrupt mask and timeouts,
...
static int sdhc_esp32_reset(const struct device *dev)
{
	...
	ret = sdhc_esp32_reset_ctrl(dev);
	if (ret != 0) {
		k_mutex_unlock(&sdhc_esp32_ctrl.bus_mutex);
		return ret;
	}

	sdhc_esp32_restore_after_reset(dev);
	k_msgq_purge(data->s_host_ctx.event_queue);
...
static void IRAM_ATTR sdio_esp32_isr(void *arg)
{
	...
	for (int slot = 0; slot < SOC_SDMMC_NUM_SLOTS; slot++) {
		 ...
		 */
		sdmmc_ll_enable_interrupt(sdio_hw, io_bit, false);
		sdmmc_ll_clear_interrupt(sdio_hw, io_bit);
...
/*
 * Bring up the parts of the controller that are shared by both slots. This
 * runs once: a second slot initialising later reuses the clock, the interrupt
 * and the DMA set up here, and resetting or re-allocating them would disturb
 * a slot that is already running.
 */
static int sdhc_esp32_controller_init(const struct device *dev)
{
	const struct sdhc_esp32_config *cfg = dev->config;
	struct sdhc_esp32_data *data = dev->data;
	sdmmc_dev_t *sdio_hw = (sdmmc_dev_t *)cfg->sdio_hw;
	...
	/* Enable interrupts, sdio interrupt is enabled only when in use */
	sdmmc_ll_enable_interrupt(
		sdio_hw,
		SDMMC_INTMASK_CD | SDMMC_INTMASK_CMD_DONE | SDMMC_INTMASK_DATA_OVER |
...
static int sdhc_esp32_init(const struct device *dev)
{
	...
	ret = sdhc_esp32_controller_init(dev);
				       ...
				       int sources, void *user_data)
{
	...
	if (sources & SDHC_INT_SDIO) {
		sdmmc_ll_clear_interrupt(sdio_hw, io_bit);
		sdmmc_ll_enable_interrupt(sdio_hw, io_bit, true);
	} else {
		sdmmc_ll_enable_interrupt(sdio_hw, io_bit, false);
	}
