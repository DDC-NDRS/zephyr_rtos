...
static int sdhc_spi_write_data(const struct device* dev, struct sdhc_data* data) {
    ...
    while (remaining--) {
        ...
        if ((resp & 0x0F) != SD_SPI_RESPONSE_ACCEPTED) {
            if ((resp & 0x0F) == SD_SPI_RESPONSE_CRC_ERR) {
                return (-EILSEQ);
            }
            else if ((resp & 0x0F) == SD_SPI_RESPONSE_WRITE_ERR) {
                return (-EIO);
            }
...
static int sdhc_spi_request(const struct device* dev,
    ...
    struct sdhc_command const stop_cmd = {
