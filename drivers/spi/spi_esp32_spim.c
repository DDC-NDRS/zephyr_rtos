...
static esp_err_t spi_esp32_create_sleep_retention_cb(void* arg) {
    ...
    return sleep_retention_entries_create(spi_reg_retention_info[idx].entry_array,
                                          spi_reg_retention_info[idx].array_size,
                                          REGDMA_LINK_PRI_GPSPI,
                                          spi_reg_retention_info[idx].module_id);
}
...
static void spi_esp32_sleep_retention_init(const struct device* dev) {
    ...
    sleep_retention_module_init_param_t init_param = {
        .cbs = {
            .create = {
                .handle = spi_esp32_create_sleep_retention_cb,
                .arg    = (void*)dev
            }
        },
