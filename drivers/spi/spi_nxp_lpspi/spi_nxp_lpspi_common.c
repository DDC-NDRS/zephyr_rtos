...
static uint8_t lpspi_calc_delay_scaler(uint32_t desired_delay_ns,
    ...
    delay_cycles = z_max(delay_cycles, min_cycles);
    ...
    /* Don't overflow */
    delay_cycles = z_min(delay_cycles, UINT8_MAX);
...
int lpspi_configure(const struct device* dev, const struct spi_config* spi_cfg) {
    ...
    lpspi->TCR = LPSPI_TCR_CPOL((operation & SPI_MODE_CPOL) != 0) |
