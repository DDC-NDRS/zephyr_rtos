...
#define INST_MAP(nodelabel, node_id) {              \
    .label = STRINGIFY(nodelabel),                  \
    .name  = DEVICE_DT_NAME(node_id)                \
},

#define INST_MAP_FROM_NODE_ID(node_id) {            \
    .label = DEVICE_DT_NAME(node_id),               \
    .name  = DEVICE_DT_NAME(node_id)                \
},
 ...
 * runtime.
 */
static struct spi_shell_map maps[] = {
...
static int spi_buses_init(void) {
    ...
    while (true) {
        ...
        maps[num_maps++] = (struct spi_shell_map) {
            .label = dev->name,
            .name  = dev->name,
        };
 ...
 */
static struct spi_dt_spec* find_spec_by_label(char const* label) {
...
static int cmd_spi_transceive(const struct shell* sh, size_t argc, char** argv) {
    uint8_t rx_buffer[MAX_SPI_BYTES];
    uint8_t tx_buffer[MAX_SPI_BYTES];

    struct spi_dt_spec* spec = find_spec_by_label(argv[TXRX_ARGV_SPI_DEV]);

    if (spec == NULL) {
        shell_error(sh, "device %s not found.", argv[TXRX_ARGV_SPI_DEV]);
        return (-ENODEV);
    }
    ...
    if (ret < 0) {
        shell_error(sh, "spi_transceive returned %d", ret);
        return (ret);
    }

    shell_print(sh, "TX:");
    shell_hexdump(sh, tx_buffer, bytes_to_send);

    shell_print(sh, "RX:");
    shell_hexdump(sh, rx_buffer, bytes_to_send);
...
static int cmd_spi_conf(const struct shell* sh, size_t argc, char** argv) {
    ...
    if (spec == NULL) {
        shell_error(sh, "device %s not found.", argv[CONF_ARGV_SPI_DEV]);
        return (-ENODEV);
    }
    ...
    if (!IN_RANGE(frequency, 100 * 1000, 80 * 1000 * 1000)) {
        shell_error(sh, "frequency must be between 100000  and 80000000");
        return (-EINVAL);
    }
    ...
    while (*opts != '\0') {
        switch (*opts) {
            ...
            default :
                ...
                shell_error(sh, "invalid setting %c", *opts);
        }
...
static int cmd_spi_conf_cs(const struct shell* sh, size_t argc, char** argv) {
    struct spi_dt_spec* spec = find_spec_by_label(argv[CS_ARGV_SPI_DEV]);

    if (spec == NULL) {
        shell_error(sh, "device %s not found.", argv[CS_ARGV_SPI_DEV]);
        return (-ENODEV);
    }
    ...
    if (dev == NULL) {
        shell_error(sh, "device %s not found.", argv[CS_ARGV_GPIO_DEV]);
        return (-ENODEV);
    }

    int pin = strtol(argv[CS_ARGV_GPIO_PIN], &endptr, 10);
    if (endptr == argv[CS_ARGV_GPIO_PIN] || (pin < 0)) {
        shell_error(sh, "invalid pin number: %s", argv[CS_ARGV_GPIO_PIN]);
        return (-EINVAL);
    }
    ...
    if (argc == (CS_ARGV_GPIO_FLAGS + 1)) {
        ...
        if (endptr == argv[CS_ARGV_GPIO_FLAGS]) {
            shell_error(sh, "invalid gpio flags: %s", argv[CS_ARGV_GPIO_FLAGS]);
            return (-EINVAL);
        }
