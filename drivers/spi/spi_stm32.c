...
static int spi_stm32_dma_tx_load(const struct device* dev, uint8_t const* buf, size_t len) {
    ...
    blk_cfg->dest_address = ll_dma_get_reg_addr(spi, SPI_STM32_DMA_TX);
...
static int spi_stm32_dma_rx_load(const struct device* dev, uint8_t* buf, size_t len) {
    ...
    blk_cfg->source_address = ll_dma_get_reg_addr(spi, SPI_STM32_DMA_RX);
...
static void spi_stm32_dma_rx_done(const struct device* dev, const struct spi_config* config) {
    ...
    __maybe_unused size_t dfs = bits2bytes(config->operation);
    ...
    while (ll_spi_dma_busy(spi) && LL_SPI_IsEnabled(spi)) {
        if (DT_HAS_COMPAT_STATUS_OKAY(st_stm32h7_spi)) {
            uint32_t width = SPI_WORD_SIZE_GET(config->operation);
...
static size_t spi_stm32_send_next_frame(SPI_TypeDef* spi, struct spi_stm32_data* data,
    ...
    if (DT_HAS_COMPAT_STATUS_OKAY(st_stm32h7_spi) &&
        ((dfs == 4U) ||
        ((dfs == 2U) && (fifo_space >= 2U) && (ctx->tx.len >= 2)) ||
        ((dfs == 1U) && (fifo_space >= 4U) && (ctx->tx.len >= 4)))) {
        ...
        if ((dfs == 2U) ||
            ((dfs == 1U) && (fifo_space >= 2U) && (ctx->tx.len >= 2))) {
            if (spi_context_tx_buf_on(ctx)) {
...
static size_t spi_stm32_read_next_frame(SPI_TypeDef* spi, struct spi_stm32_data* data,
                                        size_t fifo_space) {
    struct spi_context* ctx = &data->ctx;
    size_t const dfs = bits2bytes(ctx->config->operation);
    uint32_t rx_frame;
    ...
    if (DT_HAS_COMPAT_STATUS_OKAY(st_stm32h7_spi) &&
        ((dfs == 4U) ||
        ((dfs == 2U) && (fifo_space >= 2U) && (ctx->rx.len >= 2)) ||
        ((dfs == 1U) && (fifo_space >= 4U) && (ctx->rx.len >= 4)))) {
        ...
        if ((dfs == 2U) ||
            ((dfs == 1U) && (fifo_space >= 2U) && (ctx->rx.len >= 2))) {
...
static void spi_stm32_send_fifo(SPI_TypeDef* spi, struct spi_stm32_data* data) {
    size_t nb_sent;
    size_t fifo_space = data->fifo_threshold;
...
static void spi_stm32_shift_s(SPI_TypeDef* spi, struct spi_stm32_data* data) {
    ...
    if ((dir != STM32_SPI_HALF_DUPLEX_RX) &&
    ...
    if ((dir != STM32_SPI_HALF_DUPLEX_TX) &&
...
static void spi_stm32_msg_start(const struct device* dev, bool is_rx_empty) {
    ...
    #if defined(CONFIG_SPI_STM32_INTERRUPT)
    if (DT_HAS_COMPAT_STATUS_OKAY(st_stm32h7_spi) &&
        (!IS_ENABLED(CONFIG_SPI_RTIO) || (LL_SPI_GetMode(spi) == LL_SPI_MODE_MASTER))) {
...
static void spi_stm32_iodev_msg_start(const struct device* dev, struct spi_config* config,
    ...
    struct spi_stm32_data* data = dev->data;
    struct spi_context* ctx = &data->ctx;
    const size_t dfs = bits2bytes(config->operation);
    ...
    ctx->tx_buf   = tx_buf;
    ctx->rx_buf   = rx_buf;
    ctx->tx.len   = (tx_buf != NULL) ? size : 0;
    ctx->rx.len   = (rx_buf != NULL) ? size : 0;
    ctx->tx.count = (tx_buf != NULL) ? 1 : 0;
    ctx->rx.count = (rx_buf != NULL) ? 1 : 0;
...
static void spi_stm32_iodev_submit(const struct device* dev, struct rtio_iodev_sqe* iodev_sqe) {
    ...
    if (spi_rtio_submit(rtio_ctx, iodev_sqe)) {
        ret = spi_stm32_iodev_prepare_start(dev);
        if (ret == 0) {
            spi_stm32_iodev_start(dev);
        }
        else {
...
static void spi_stm32_complete(const struct device* dev, int status) {
    ...
    /* Flush RX buffer */
    while (DT_HAS_COMPAT_STATUS_OKAY(st_stm32_spi_fifo) &&
           ll_rx_is_not_empty(spi)) {
    ...
    spi_operation_t operation = ctx->config->operation;
    ...
    if (LL_SPI_GetMode(spi) == LL_SPI_MODE_MASTER) {
        while (ll_spi_is_busy(spi) && LL_SPI_IsEnabled(spi)) {
            ...
            if (DT_HAS_COMPAT_STATUS_OKAY(st_stm32h7_spi)) {
                uint32_t width = SPI_WORD_SIZE_GET(operation);
    ...
    #ifdef CONFIG_SPI_PERIPHERAL
    bool peripheral_hd_tx = spi_context_is_peripheral(ctx) &&
                            (operation & SPI_HALF_DUPLEX)  &&
...
static int spi_stm32_configure(const struct device* dev,
    ...
    if (!spi_stm32_is_data_width_supported(dev, SPI_WORD_SIZE_GET(operation))) {
        return (-ENOTSUP);
    }

    /* configure the frame format Motorola (default) or TI */
    if ((operation & SPI_FRAME_FORMAT_TI) == SPI_FRAME_FORMAT_TI) {
    ...
    if (operation & SPI_CRC_ENABLE) {       /* #CUSTOM@NDRS */
    ...
    if (spi_cs_is_gpio(config) || cfg->soft_nss) {
        ...
        if ((operation & SPI_OP_MODE_PERIPHERAL) != 0U) {
    ...
    #if ANY_SPI_DATA_WIDTH_IS(STM32_SPI_DATA_WIDTH_FULL_4_TO_32_BIT) || \
        ANY_SPI_DATA_WIDTH_IS(STM32_SPI_DATA_WIDTH_FULL_4_TO_16_BIT)
    LL_SPI_SetDataWidth(spi, table_datawidth[SPI_WORD_SIZE_GET(operation) -
                        STM32_SPI_DATA_WIDTH_MIN]);
    #else
    LL_SPI_SetDataWidth(spi, table_datawidth[bits2bytes(operation) - 1U]);
    #endif
    ...
    if (!use_dma) {
        ...
        uint8_t dfs = DIV_ROUND_UP(SPI_WORD_SIZE_GET(operation), 8U);
    ...
    if (operation & SPI_STPM3X_DRV) {       /* #CUSTOM@NDRS */
    ...
    LOG_DBG("Installed config %p: freq %uHz (div = %u), mode %u/%u/%u, peripheral %u",
            ...
            (SPI_MODE_GET(operation) & SPI_MODE_CPOL) ? 1 : 0,
            (SPI_MODE_GET(operation) & SPI_MODE_CPHA) ? 1 : 0,
            (SPI_MODE_GET(operation) & SPI_MODE_LOOP) ? 1 : 0,
            config->peripheral);
...
static int32_t spi_stm32_count_bufset_frames(const struct spi_config* config,
    ...
    size_t count = bufs->count;
    struct spi_buf const* buffers = bufs->buffers;

    for (size_t i = 0; i < count; i++) {
        num_bytes += buffers[i].len;
    }

    size_t dfs = bits2bytes(config->operation);
...
int spi_stpm3x_init(struct spi_dt_spec const* spec,
    ...
    ret = spi_stm32_configure(spec->bus, &spec->config, false, false);
    if (ret == 0) {
        data->State = HAL_SPI_STATE_READY;
...
static int wait_dma_rx_tx_done(const struct device* dev) {
    ...
    if (IS_ENABLED(CONFIG_SPI_PERIPHERAL) &&
        spi_context_is_peripheral(&data->ctx)) {
        timeout = K_FOREVER;
    }
    else {
        timeout = K_MSEC(1000);
    ...
    while (true) {
        ret = k_sem_take(&data->status_sem, timeout);
        if (ret != 0) {
            return (ret);
        }
...
static bool spi_buf_set_in_nocache(const struct spi_buf_set* bufs) {
    ...
    struct spi_buf const* buffers = bufs->buffers;
    for (size_t i = 0; i < count; i++) {
        struct spi_buf const* buf = &buffers[i];
...
static void spi_stm32_dma_callback_async(const struct device* spi_dev, uint32_t channel, int status) {
    ...
    if ((data->status_flags & SPI_STM32_DMA_DONE_FLAG) == SPI_STM32_DMA_DONE_FLAG) {
        ...
        if (dma_len != 0U) {
            data->tx_len = data->ctx.tx.len;
            data->rx_len = data->ctx.rx.len;
...
static void spi_stm32_dma_callback_sync(const struct device* spi_dev, uint32_t channel, int status) {
    struct spi_stm32_data* data = spi_dev->data;
...
__maybe_unused
static void spi_stm32_dma_callback(const struct device* dma_dev, void* arg, uint32_t channel, int status) {
    ...
    #ifdef CONFIG_SPI_RTIO
    spi_stm32_dma_callback_async(spi_dev, channel, status);
    return;
    #else
    ...
    spi_stm32_dma_callback_sync(spi_dev, channel, status);
    #endif /* CONFIG_SPI_RTIO */
}
...
static int spi_stm32_ll_transceive_dma(const struct device* dev,
    ...
    #ifdef CONFIG_SPI_PERIPHERAL
    if (spi_context_is_peripheral(ctx) && (ret == 0)) {
        ret = ctx->recv_frames;
    }
    #endif /* CONFIG_SPI_PERIPHERAL */
...
static int spi_stm32_ll_transceive(const struct device* dev,
    ...
    #ifdef CONFIG_SPI_STM32_DMA
    if (use_dma) {
        ret = spi_stm32_ll_transceive_dma(dev, config);
        goto end;
    }
    #endif /* CONFIG_SPI_STM32_DMA */
...
static int spi_stm32_transceive_sync(const struct device* dev,
    ...
    ret = spi_stm32_ll_transceive(dev, config, tx_bufs, rx_bufs,
                                  false, NULL, NULL);
...
static int spi_stm32_pinctrl_apply(const struct device* dev, uint8_t id) {
    ...
    ret = pinctrl_apply_state(config->pcfg, id);
    if ((id == PINCTRL_STATE_SLEEP) && (ret == -ENOENT)) {
...
static int spi_stm32_pm_action(const struct device* dev, enum pm_device_action action) {
    ...
    switch (action) {
        case PM_DEVICE_ACTION_RESUME :
            ...
            ret = spi_stm32_pinctrl_apply(dev, PINCTRL_STATE_DEFAULT);
            if (ret < 0) {
                return (ret);
            }
        ...
        case PM_DEVICE_ACTION_SUSPEND :
            /* Stop device clock. */
            ret = clock_control_off(clk, (clock_control_subsys_t)&config->pclken[0]);
            if (ret != 0) {
            ...
            ret = spi_stm32_pinctrl_apply(dev, PINCTRL_STATE_SLEEP);
            return (ret);
...
static int spi_stm32_init(const struct device* dev) {
    ...
    ret = pm_device_driver_init(dev, spi_stm32_pm_action);
