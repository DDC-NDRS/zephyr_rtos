...
static uint8_t spi_stm32_send_next_frame(SPI_TypeDef* spi, struct spi_stm32_data* data) {
    ...
    uint8_t const dfs = bits2bytes(ctx->config->operation);
...
static void spi_stm32_shift_s(SPI_TypeDef* spi, struct spi_stm32_data* data) {
    ...
    if (ll_tx_is_not_full(spi) && spi_context_tx_on(ctx)) {
        spi_stm32_send_next_frame(spi, data);
    }
    else {
    ...
    if (ll_rx_is_not_empty(spi) && spi_context_rx_buf_on(ctx)) {
        spi_stm32_read_next_frame(spi, data);
    }
}
...
static void spi_stm32_iodev_msg_start(const struct device* dev, struct spi_config* config,
    ...
    ctx->tx.current = &current_tx;
    ctx->rx.current = &current_rx;
...
static int spi_stm32_ll_transceive_dma(const struct device* dev,
    ...
    while ((ctx->rx.len > 0) || (ctx->tx.len > 0)) {
        size_t dma_len;
        ...
        while (ll_spi_dma_busy(spi) && LL_SPI_IsEnabled(spi)) {
            #if DT_HAS_COMPAT_STATUS_OKAY(st_stm32h7_spi)
            uint32_t width = SPI_WORD_SIZE_GET(ctx->config->operation);
        ...
        if (transfer_dir == LL_SPI_FULL_DUPLEX) {
            spi_context_update_tx(ctx, dfs, dma_len);
            spi_context_update_rx(ctx, dfs, dma_len);
        }
        else if (transfer_dir == LL_SPI_HALF_DUPLEX_TX) {
            spi_context_update_tx(ctx, dfs, dma_len);
        }
