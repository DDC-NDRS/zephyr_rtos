...
static int fs26_spi_transceive(const struct spi_dt_spec* spi,
    ...
    ret = spi_transceive_dt(spi, &spi_tx_set, &spi_rx_set);
    if (ret != 0) {
        goto error;
    }
...
static int fs26_getreg(const struct spi_dt_spec* spi, uint8_t addr,
    ...
    ret = fs26_spi_transceive(spi, &tx_frame, rx_frame);

    return ret;
}

/**
 * @brief Set @p val value in register with address @p addr
 *
 * @param spi SPI specs for interacting with the device
 * @param addr Register address
 ...
 * @return 0 on success, error code otherwise
 */
static int fs26_setreg(const struct spi_dt_spec* spi, uint8_t addr, uint16_t val) {
    struct fs26_spi_tx_frame tx_frame = {
        .addr = addr,
        .write = true,
        .data = val
    ...
    ret = fs26_spi_transceive(spi, &tx_frame, NULL);
...
static int fs26_wd_refresh(const struct device* dev) {
    ...
    struct spi_dt_spec const* spec = &config->spi;
    ...
    if (config->wd_type == FS26_WD_SIMPLE) {
        if (fs26_setreg(spec, FS26_FS_WD_ANSWER, data->token) != 0) {
        ...
        if (fs26_getreg(spec, FS26_FS_WD_TOKEN, &rx_frame) != 0) {
            ...
            if (fs26_setreg(spec, FS26_FS_WD_ANSWER, answer) != 0) {
                LOG_ERR("Failed to write answer");
                ret = -EIO;
            }
        }
    ...
    if (ret == 0) {
        if (fs26_getreg(spec, FS26_FS_GRL_FLAGS, &rx_frame) == 0) {
            if ((rx_frame.data & FS_WD_G_MASK) == FS_WD_G) {
                if (fs26_getreg(spec, FS26_FS_DIAG_SAFETY1, &rx_frame) == 0) {
...
static int fs26_poll_for_init_fs_state(const struct device* dev) {
    ...
    do {
        if (fs26_getreg(spec, FS26_FS_STATES, &rx_frame) == 0) {
            val = rx_frame.data;
        }
        k_sleep(K_MSEC(1));
        now = k_uptime_get();
    } while ((now < timeout) && (val & FS_STATES_MASK) != FS_STATES_INIT_FS);
...
static int fs26_goto_init_fs_state(const struct device* dev) {
    ...
    if (fs26_getreg(spec, FS26_FS_STATES, &rx_frame) == 0) {
        ...
        if (current_state < FS_STATES_INIT_FS) {
            ...
            ret = fs26_poll_for_init_fs_state(dev);
        }
    }
...
static int wdt_nxp_fs26_install_timeout(const struct device* dev,
    ...
    for (i = 0; i < ARRAY_SIZE(fs26_dc_closed_num); i++) {
        window_min = ((uint32_t)fs26_dc_closed_num[i]
...
static int wdt_nxp_fs26_disable(const struct device* dev) {
    ...
    if (fs26_getreg(spec, FS26_FS_WDW_DURATION, &rx_frame) != 0) {
        return -EIO;
    }
...
static void wdt_nxp_fs26_int_thread(void* p1, void* p2, void* p3) {
    ...
    while (true) {
        ...
        if ((fs26_getreg(spec, FS26_FS_GRL_FLAGS, &rx_frame) == 0) &&
            ...
            if ((fs26_getreg(spec, FS26_FS_DIAG_SAFETY1, &rx_frame) == 0) &&
                ((rx_frame.data & BAD_WD_TIMING) != 0)) {

                /* Clear flag */
                val = BAD_WD_TIMING;
                fs26_setreg(spec, FS26_FS_DIAG_SAFETY1, val);
...
static int wdt_nxp_fs26_init(const struct device* dev) {
    ...
    if ((rx_frame.data & DBG_MODE_MASK) == DBG_MODE) {
        if (IS_ENABLED(CONFIG_WDT_NXP_FS26_EXIT_DEBUG_MODE)) {
            ...
            val = rx_frame.data | EXIT_DBG_MODE;
    ...
    val = BAD_WD_DATA | BAD_WD_TIMING | ABIST2_PASS | ABIST2_DONE |
    ...
    /* OVUV_SAFE_REACTION1 */
    val = VMON_PRE_OV_FS_REACTION_NO_EFFECT  |
    ...
    fs26_setreg(spec, FS26_FS_I_OVUV_SAFE_REACTION1, val);
    fs26_setreg(spec, FS26_FS_I_NOT_OVUV_SAFE_REACTION1, ~val);

    /* OVUV_SAFE_REACTION2 */
    val = VMON_EXT_OV_FS_REACTION_NO_EFFECT  |
    ...
    fs26_setreg(spec, FS26_FS_I_OVUV_SAFE_REACTION2, val);
    fs26_setreg(spec, FS26_FS_I_NOT_OVUV_SAFE_REACTION2, ~val);

    /* FS_I_SAFE_INPUTS */
    val = FCCU_CFG_NO_MONITORING | ERRMON_ACK_TIME_32MS;

    fs26_setreg(spec, FS26_FS_I_SAFE_INPUTS, val);
    fs26_setreg(spec, FS26_FS_I_NOT_SAFE_INPUTS, ~val);

    /* FS_I_FSSM */
    val = FLT_ERR_REACTION_NO_EFFECT | CLK_MON_DIS | DIS8S;

    fs26_setreg(spec, FS26_FS_I_FSSM, val);
    fs26_setreg(spec, FS26_FS_I_NOT_FSSM, ~val);
    ...
    if (data->token != FS26_FS_WD_TOKEN_DEFAULT) {
        ...
        fs26_setreg(spec, FS26_FS_WD_TOKEN, data->token);
    }

    /* Mask all Fail-Safe interrupt sources except for watchdog bad refresh */
    val = ~BAD_WD_M;
    fs26_setreg(spec, FS26_FS_INTB_MASK, val);

    /* Mask all main interrupt sources */
    val = 0xFFFF;
    fs26_setreg(spec, FS26_M_TSD_MSK , val);
    fs26_setreg(spec, FS26_M_REG_MSK , val);
    fs26_setreg(spec, FS26_M_VSUP_MSK, val);
    fs26_setreg(spec, FS26_M_WIO_MSK , val);
    fs26_setreg(spec, FS26_M_COM_MSK , val);
    ...
    if (fs26_getreg(&config->spi, FS26_FS_STATES, &rx_frame) == 0) {
...
#define FS26_WDT_DEVICE_INIT(n)                                 \
    ...
    static const struct wdt_nxp_fs26_config wdt_nxp_fs26_config_##n = { \
        .spi = SPI_DT_SPEC_INST_GET(n, SPI_OP_MODE_CONTROLLER | SPI_MODE_CPHA | \
