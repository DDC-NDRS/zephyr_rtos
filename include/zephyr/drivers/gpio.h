 ...
 *
 * @note @p pin is checked against @ref GPIO_MAX_PINS_PER_PORT before it is
 * used as a shift count. Drivers must use this helper instead of testing
 * `port_pin_mask & BIT(pin)` directly, as @p pin may originate from an
 * untrusted caller.
 *
 * @param port_pin_mask Mask identifying the pins supported by the controller,
 *                      i.e. @ref gpio_driver_config.port_pin_mask.
 * @param pin Pin number to check.
 *
 * @retval true If @p pin is in range and supported by the controller.
 * @retval false Otherwise.
 */
static inline bool gpio_port_pin_is_supported(gpio_port_pins_t port_pin_mask, gpio_pin_t pin) {
    if (pin >= GPIO_MAX_PINS_PER_PORT) {
        return (false);
    }

    return ((port_pin_mask & (gpio_port_pins_t)BIT(pin)) != 0U);
}
 ...
 */
typedef void (*gpio_callback_handler_t)(struct device const* port,
 ...
 */
typedef int (*gpio_api_pin_configure_t)(struct device const* port, gpio_pin_t pin,
                                        gpio_flags_t flags);
 ...
 */
typedef int (*gpio_api_pin_get_config_t)(struct device const* port, gpio_pin_t pin,
                                         gpio_flags_t* flags);
 ...
 */
typedef int (*gpio_api_port_get_raw_t)(struct device const* port,
 ...
 */
typedef int (*gpio_api_port_set_masked_raw_t)(struct device const* port,
 ...
 */
typedef int (*gpio_api_port_set_bits_raw_t)(struct device const* port,
                                            gpio_port_pins_t pins);
 ...
 */
typedef int (*gpio_api_port_clear_bits_raw_t)(struct device const* port,
                                              gpio_port_pins_t pins);
 ...
 */
typedef int (*gpio_api_port_toggle_bits_t)(struct device const* port,
                                           gpio_port_pins_t pins);
 ...
 */
typedef int (*gpio_api_pin_interrupt_configure_t)(struct device const* port,
                                                  gpio_pin_t pin,
 ...
 */
typedef int (*gpio_api_manage_callback_t)(struct device const* port,
                                          struct gpio_callback* cb,
                                          bool set);
 ...
 */
typedef int (*gpio_api_port_get_direction_t)(struct device const* port, gpio_port_pins_t map,
