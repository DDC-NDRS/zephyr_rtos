					      ...
					      uint8_t n, bool is_signed)
{
	...
	if (is_signed) {
		if (!IN_RANGE(raw, -(int64_t)BIT64(num_bits - 1), (int64_t)BIT64_MASK(num_bits - 1))) {
			return -ERANGE;
		}
						...
						uint8_t n, bool is_signed)
{
	...
	val->val1 = (int32_t)arithmetic_shift_right(inp_int64 + (inp_int64 < 0 ? BIT64_MASK(n) : 0), n);

	inp_int64 -= ((int64_t)val->val1 << n);
	inp_int64 *= 1000000LL;

	val->val2 = (int32_t)arithmetic_shift_right(inp_int64 + (inp_int64 < 0 ? BIT64_MASK(n) : 0), n);
