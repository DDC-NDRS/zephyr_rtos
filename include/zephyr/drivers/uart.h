...
__subsystem struct uart_driver_api {
    ...
    int (*tx)(const struct device* dev, uint8_t const* buf, size_t len,
              int32_t timeout);
    ...
    int (*tx_u16)(const struct device* dev, uint16_t const* buf,
    ...
    int (*fifo_fill)(const struct device* dev, uint8_t const* tx_data, int len);

    #ifdef CONFIG_UART_WIDE_DATA
    int (*fifo_fill_u16)(const struct device* dev, uint16_t const* tx_data, int len);
    #endif

    /** Interrupt driven FIFO read function */
    int (*fifo_read)(const struct device* dev, uint8_t* rx_data, int const size);

    #ifdef CONFIG_UART_WIDE_DATA
    int (*fifo_read_u16)(const struct device* dev, uint16_t* rx_data, int const size);
    #endif
...
static inline int uart_fifo_fill(const struct device* dev,
                                 uint8_t const* tx_data, int size) {
    #ifdef CONFIG_UART_INTERRUPT_DRIVEN
    const struct uart_driver_api* api = (const struct uart_driver_api*)dev->api;
...
static inline int uart_fifo_fill_u16(const struct device* dev,
                                     uint16_t const* tx_data, int size) {
 ...
 * @retval -ENOTSUP If API is not enabled.
 */
static inline int uart_fifo_read(const struct device* dev, uint8_t* rx_data, int const size) {
    #ifdef CONFIG_UART_INTERRUPT_DRIVEN
    const struct uart_driver_api* api = (const struct uart_driver_api*)dev->api;
...
static inline int uart_fifo_read_u16(const struct device* dev,
                                     uint16_t* rx_data, int const size) {
    #if defined(CONFIG_UART_INTERRUPT_DRIVEN) && defined(CONFIG_UART_WIDE_DATA)
    const struct uart_driver_api* api = (const struct uart_driver_api*)dev->api;
 ...
 */
__syscall int uart_tx(const struct device* dev, uint8_t const* buf,
                      size_t len,
                      int32_t tmout_us);

static inline int z_impl_uart_tx(const struct device* dev, uint8_t const* buf,
 ...
 * @retval -errno Other negative errno value in case of failure.
 */
__syscall int uart_tx_u16(const struct device* dev, uint16_t const* buf,
