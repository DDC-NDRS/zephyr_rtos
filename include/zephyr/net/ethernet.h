...
#if defined(CONFIG_NET_L2_ETHERNET_MCAST_FILTER_COUNT) && !defined(_MSC_VER) /* #CUSTOM@NDRS */
 ...
 */
struct net_eth_addr const* net_eth_broadcast_addr(void);
...
typedef void (*net_eth_mcast_addr_cb_t)(struct net_if* iface,
                                        struct net_eth_mcast_addr const* addr,
                                        void* user_data);
...
int net_eth_mcast_addr_add(struct net_if* iface,
                           struct net_eth_addr const* addr);
...
enum ethernet_hw_caps net_eth_get_hw_capabilities(struct net_if* iface) {
    ...
    api = (struct ethernet_api const*)dev->api;
...
static inline bool net_eth_type_is_wifi(struct net_if* iface) {
    struct ethernet_context const* ctx = (struct ethernet_context*)
