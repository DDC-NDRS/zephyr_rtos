...
static size_t copy_bytes(char* dest, size_t dest_size, char const* src, size_t src_size) {
    size_t bytes_to_copy;

    bytes_to_copy = z_min(dest_size, src_size);
...
static size_t random_offset(size_t stack_size) {
     ...
     * See #6493
     */
    size_t const fuzz = random_val % CONFIG_STACK_POINTER_RANDOM;
...
static inline void set_stack_info(struct k_thread* new_thread, char* buf_start,
    ...
    new_thread->stack_info.usage.unused_threshold =
        (CONFIG_THREAD_RUNTIME_STACK_SAFETY_DEFAULT_UNUSED_THRESHOLD_PCT * buf_size) / 100;
    #endif
    #else
    ARG_UNUSED(new_thread);
    ARG_UNUSED(buf_start);
...
static inline void init_thread_name(struct k_thread* thread, char const* name) {
    #ifdef CONFIG_THREAD_NAME
...
int k_thread_runtime_stack_safety_full_check(const struct k_thread* thread,
    ...
    rv = z_stack_space_get((uint8_t const*)thread->stack_info.start,
