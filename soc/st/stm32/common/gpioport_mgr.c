 ...
 */
static struct device const* const gpio_ports[] = {
...
static void ll_gpio_set_pin_pull(GPIO_TypeDef* GPIOx, uint32_t Pin, uint32_t Pull) {
     ...
     */
    uint32_t const gpio = (GPIOx == GPIOA) ? LL_PWR_GPIO_A : LL_PWR_GPIO_B;
...
struct device const* stm32_gpioport_get(uint32_t port_index) {
...
__maybe_unused
static int stm32_gpioport_init(const struct device* dev) {
    int ret;
