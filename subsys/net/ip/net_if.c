...
int net_if_set_link_addr_locked(struct net_if* iface,
                                uint8_t const* addr, uint8_t len,
...
static enum net_l2_flags l2_flags_get(struct net_if* iface) {
    struct net_l2 const* l2 = net_if_l2(iface);
...
void net_if_ipv6_dad_failed(struct net_if* iface, struct net_in6_addr const* addr) {
    struct net_if_addr* ifaddr;
    uint32_t timeout;
...
bool net_if_ipv6_addr_rm(struct net_if* iface, struct net_in6_addr const* addr) {
    struct net_if_addr* ifaddr;
    struct net_if_ipv6 const* ipv6;
    bool result = true;
    int ret;
...
bool net_if_ipv6_maddr_rm(struct net_if* iface, struct net_in6_addr const* addr) {
    bool ret = false;
    struct net_if_ipv6* ipv6;
    int refcount;
...
struct net_if_mcast_addr* net_if_ipv6_maddr_lookup(struct net_in6_addr const* addr,
                                                   struct net_if** iface) {
    ARG_UNUSED(addr);
    ARG_UNUSED(iface);
...
static void ipv4_addr_rm_all(struct net_if* iface, struct net_if_ipv4* ipv4) {
    ARRAY_FOR_EACH(ipv4->unicast, i) {
        ...
        ipv4->unicast[i].netmask.s_addr_be = 0;
    }
}
...
int net_if_ipv4_route_add(struct net_if* iface, struct net_in_addr const* addr, uint8_t mask_len,
                          struct net_in_addr const* nexthop, uint32_t lifetime) {
    #if defined(CONFIG_NET_IPV4_ROUTE)
    struct net_route_entry *route;
