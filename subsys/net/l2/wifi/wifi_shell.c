				 ...
				 enum wifi_iface_mode iface_mode)
{
	...
	while ((opt = sys_getopt_long(argc, argv, "s:p:k:e:w:b:c:m:t:a:B:K:S:T:A:V:I:P:g:Rh:i:",
		...
		case 'K':
			...
			break;
		case 'S':
			params->wpa3_ent_mode = (uint8_t)atoi(state->optarg);
		...
		case 'e':
			...
			params->server_cert_domain_exact_len =
					(uint8_t)strlen(params->server_cert_domain_exact);
			break;
		case 'x':
			params->server_cert_domain_suffix = state->optarg;
			params->server_cert_domain_suffix_len =
					(uint8_t)strlen(params->server_cert_domain_suffix);
			break;
		case 'h':
			shell_help(sh);
			return -ENOEXEC;
				    ...
				    char *argv[])
{
	...
	params.setup.twt_mantissa = (uint16_t)ceil(twt_mantissa_scale * scale);
	params.setup.twt_exponent = (uint8_t)exponent;
				 ...
				 struct wifi_twt_params *params)
{
	...
	if (params->setup.twt_interval) {
		...
		params->setup.twt_exponent = (uint8_t)exponent;
		   ...
		   (params->setup.twt_mantissa != 0)) {
		params->setup.twt_interval = (uint64_t)floor(ldexp(params->setup.twt_mantissa,
			       ...
			       char *argv[])
{
	...
	if (state->optind == argc) {
		...
		regd.country_code[0] = args[0];
		regd.country_code[1] = args[1];
		regd.force = force;
